...
### Task 3: Implement REST RPC mapping and SSE delivery
...
Cover health 200/503, pending list, unknown cursor 400, ACK 200/404, lifecycle route parsing, method-not-allowed 405, unknown route 404, invalid JSON 400, duplicate/conflicting `Content-Length` 400, and body overflow 413.
...
`readJsonBody()` must reject multiple `content-length` values, non-decimal lengths, declared lengths above the configured limit, streaming bodies that exceed the limit, empty bodies, arrays, and malformed JSON. `createApiHandler()` must set `Cache-Control: no-store` and `application/json; charset=utf-8`, map `GatewayRpcError` codes to 400/404/409/503/504/502, and never return credentials or exception stacks.
...
### Task 4: Register the Bridge hooks and authenticated plugin route
...
Use a temporary SQLite file and call the runtime handlers directly:

```js
const runtime = createBridgeRuntime({ databasePath, logger: silentLogger, dispatchGatewayMethod: fakeDispatch });
...
Add a fake store whose `enqueue()` throws and assert `onBeforeDispatch()` still returns `{ handled: true }`, health is degraded, and the logger receives only a controlled code/event id rather than the message text.
...
`createBridgeRuntime()` owns the store, health state, correlation buffer, SSE hub, RPC facade, and API handler. `onMessageReceived()` must only capture synchronously. `onBeforeDispatch()` merges the captured event, normalizes it, enqueues it, recovers health on success, and returns `{ handled: true }` in both success and error paths. `close()` clears timers, closes SSE clients, and closes SQLite exactly once.
...
`createChannelGatewayPlugin({ dispatchGatewayMethod })` returns `definePluginEntry(...)`. In `register(api)`, return immediately unless `api.registrationMode === "full"`; resolve config defaults from `api.pluginConfig`; register both hooks with high priority and a 5-second timeout; register the authenticated prefix route with `gatewayRuntimeScopeSurface: "trusted-operator"`; register lifecycle cleanup. `index.js` imports the public SDK dispatcher and exports the production plugin.
...
git commit -m "Intercept channel ingress before model dispatch" -m "Register a fail-closed awaited hook, rich metadata correlation, authenticated admin-scoped API route, and lifecycle cleanup as one OpenClaw plugin runtime.

Constraint: message_received is fire-and-forget, so enrichment capture must be synchronous
Confidence: high
Scope-risk: moderate
Directive: before_dispatch must return handled even when persistence fails
Tested: node --test bridge-runtime and plugin-registration; npm run check"
```

### Task 5: Build the standalone launcher and exact-version package discovery
...
The built-in candidate list is Discord, Slack, and WhatsApp at `2026.6.11`. Resolve only direct package roots under the service `node_modules`, read each `package.json` and `openclaw.plugin.json`, and compare both package version and manifest channel id. Parse `CHANNEL_GATEWAY_PLUGIN_PATHS` with the platform delimiter and require absolute paths with a manifest. Do not scan arbitrary user directories.
...
```js
{
  ...
  plugins: {
    ...
    entries: {
      "channel-gateway": { enabled: true, hooks: { allowConversationAccess: true, timeouts: { before_dispatch: 5000 } }, config: { databasePath } },
...
### Task 6: Apply and verify the pinned rich-before-dispatch Host patch
...
### Task 7: Add deployment, license evidence, documentation, and real standalone smoke
...
4. wait for `/healthz`, then call authenticated `/api/v1/health`, `/api/v1/events`, and `/api/v1/channels`;
...
Expected: PASS; `/api/v1/channels` contains installed Discord, Slack, and WhatsApp surfaces in unconfigured/stopped states rather than crashing startup.
...
- Telegram/Discord/Slack/WhatsApp/Teams/Matrix/Signal onboarding boundary using native OpenClaw commands;
...
Run: `rg -n "REST|SSE|ACK|CHANNEL_GATEWAY_TOKEN|Discord|Slack|WhatsApp|upgrade|license|SBOM|tenant" README.md`
