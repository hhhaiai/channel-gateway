...
- 生产安装使用 **Node.js 22.22.2**。OpenClaw 本身接受 22.19+，但当前锁定树里的
...
`18789`。容器以非 root `node` 用户运行。健康检查调用无需认证的 OpenClaw `/healthz`；Bridge/SQLite
...
确认后安装到服务树之外的固定绝对目录，再显式加载：

```bash
sudo mkdir -p /opt/channel-plugins/qq
sudo npm install --prefix /opt/channel-plugins/qq @openclaw/qqbot@2026.6.11
...
AUTH="Authorization: Bearer $CHANNEL_GATEWAY_TOKEN"

curl "$BASE/healthz"
curl -H "$AUTH" "$BASE/api/v1/health"
curl -H "$AUTH" "$BASE/api/v1/channels?probe=true"
curl -H "$AUTH" "$BASE/api/v1/links"
curl -H "$AUTH" "$BASE/api/v1/events?limit=100"
...
curl -N -H "$AUTH" -H 'Last-Event-ID: evt_122' \
