...
> 插件作为 connector kernel，不复制 任意 OpenClaw provider 协议实现。Bridge
...
| `events` 表 | payload JSON、`pending/acked/failed`、创建/更新/ACK/失败时间 | 外部控制面只读取 pending 并 ACK；`failed` 是未暴露为 REST/SSE 操作的 EventStore 内部终态。 |
...
| `/api/v1/delivery/status` | GET | 账号/Channel backlog、失败、retry 和 rate-limit/cooldown 的脱敏投影。 |
| `/api/v1/links` | GET | 已编译 link 的安全摘要和 delivery counts；不回显 endpoint `to`。 |
| `/api/v1/links/config` | GET/PUT | 读取/原子更新完整 links 与并发 override；PUT 必须携带最新 revision，成功后必须重启。 |
...
- delivery 先清理已 `sent/failed` 且父 event 已过 ACK/failed TTL 的 relation，再删除没有 delivery relation 的
  acked/failed event tombstone。默认 ACK TTL 为 5 分钟，failed TTL 为 24 小时；其中 event `failed` 是内部状态，
  当前 operator API 不提供将 event 标记为 failed 的路由。Bridge 每 60 秒调用 `prune()`。
...
# 只用 core/Telegram 能力；需任意官方插件 Channel 改为 --include=optional。
...
管理页静态入口为 `/channel-gateway`；它不包含敏感数据，页面随后使用仅存在内存中的 Bearer token 调用 `/api/v1`。完整 endpoint（含 `to`）只由受认证的 `/api/v1/links/config` 返回，原 `/api/v1/links` 保持脱敏。

`links-config-service` 的调用链是：`api-handler` → `compileLinks()` → `api.runtime.config.mutateConfigFile({ base: "source" })` → OpenClaw Host writer。它只替换 `plugins.entries.channel-gateway.config.links`，Host 负责 JSON5/include/env-ref 保留、锁、备份和原子写；保存时会比较 links revision，拒绝静默覆盖。

Bridge Runtime 仍在进程启动时调用 `compileLinks()` 并将其交给 Delivery Worker。因此配置响应的 `restartRequired:true` 是硬要求：当前 `gateway.reload.mode=off`，保存不会热更新、不会自行结束进程。

拓扑、时序、运行框架和代码调用图见 [`docs/架构与交互图.md`](docs/架构与交互图.md)。

## 9. 全量 Channel 的路由边界

`route-links.js` 按字符串 `channel`、`accountId`、`conversationId` 建索引，不包含 QQ、Telegram、WhatsApp 等平台分支。因此只要 OpenClaw 已加载某个 Channel、Bridge 能接到其入站事件并且 Gateway `send` 支持其 `to`，它即可与任何其它可用 endpoint 放在同一个 link。

安装范围与限制来自 <https://docs.openclaw.ai/channels> 及平台插件文档：core Channel（Telegram、iMessage、WebChat）、`all` profile 的 optional Channel（含固定版本 WeCom）和外置 Channel（绝对 plugin path）都可被发现；但“可加载”不等价于“支持群互通”。插件启用使用 manifest `pluginId`，links 路由使用 manifest `channelIds`；企业微信对应 `wecom-openclaw-plugin` 与 `wecom`。必须按该平台文档验证群入站、群发送、会话 ID、群准入、媒体和 reply 语义。

## 10. 网页逐 Channel 操作链路

`ui/channel-gateway.js` 内置官方 Channel metadata catalog 并渲染为独立卡片。连接后执行：

```text
浏览器内存 Bearer token
  → GET /api/v1/channels?probe=true
  → 每张 Channel 卡片状态
  → POST /api/v1/channels/:channel/start|stop|logout
  → OpenClaw Gateway RPC lifecycle
```

“添加到互通房间”只预填 links endpoint 的 `channel`，不会猜测 provider-specific `conversationId`/`to`，也不会保存任何平台凭据。Voice Call 卡片保留官方资料入口，但因其不声明 generic Channel manifest，不暴露 links 操作。

从首次启动、逐平台 onboarding、单渠道发送、真实入站取 ID、links revision 更新到 SSE/ACK 的可复制执行步骤见 [`逐渠道对接操作指南.md`](逐渠道对接操作指南.md)。外部应用应调用 `/api/v1`，不要直接依赖内部 `dispatchGatewayMethod`；内部精确方法名是 `send` 与 `channels.status/start/stop/logout`。
