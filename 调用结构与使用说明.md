...
安装范围与限制来自 <https://docs.openclaw.ai/channels>：core Channel（Telegram、iMessage、WebChat）、官方插件 Channel（`all` profile）和外置 Channel（绝对 plugin path）都可被发现；但“可加载”不等价于“支持群互通”。必须按该平台文档验证群入站、群发送、会话 ID、群准入、媒体和 reply 语义。
